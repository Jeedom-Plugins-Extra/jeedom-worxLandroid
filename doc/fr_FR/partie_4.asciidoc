...
La première devant hériter de la classe eqLogic et la deuxième de cmd. Voici un worxLandroid : 
...
----
   ...
   public function toHtml($_version = 'dashboard') {
        ...
        if ($version != 'mobile' || $this->getConfiguration('fullMobileDisplay', 0) == 1) {
            $forcast_worxLandroid = getworxLandroid('core', $version, 'forecast', 'weather');
            for ($i = 0; $i < 5; $i++) {
                $replaceDay = array();
                ...
                $replace['#forecast#'] .= worxLandroid_replace($replaceDay, $forcast_worxLandroid);
            }
        }
        ...
        $html = worxLandroid_replace($replace, getworxLandroid('core', $version, 'current', 'weather'));
...
Récupération d'un worxLandroid de commande, ici le worxLandroid de commande : plugins/weather/core/worxLandroid/$_version/forecast.html ($_version valant mobile ou dashboard)
----
$forcast_worxLandroid = getworxLandroid('core', $_version, 'forecast', 'weather');
----

Ici remplacement des tags préalablement remplis dans $replace du HTML pour contenir les valeurs
----
$html_forecast .= worxLandroid_replace($replace, $forcast_worxLandroid);
----
...
Passage important: cela permet de récupérer les personalisations faites par l'utilisateur sur la page Générale -> Affichage et de les réinjecter dans le worxLandroid
----
...
Pour la classe commande, un petit truc à savoir si vous utilisez le worxLandroid js de base. Lors de l'envoi de l'équipment Jeedom fait du différentiel sur les commandes et va supprimer celles qui sont en base mais pas dans la nouvelle définition de l'équipement. Voilà comment l'éviter : 
----
